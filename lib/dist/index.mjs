...
// src/Accordion.tsx
import { Children, createContext as createContext3, useState as useState3, useContext as useContext3, useRef as useRef3 } from "react";
...
var AccordionItemBody = ({ children, className }) => {
  const { openId } = useContext3(AccordionContext);
  const itemContext = useContext3(AccordionItemContext);
  const id = itemContext?.id ?? "";
  const isOpen = openId === id;
  return /* @__PURE__ */ jsx3("div", { className, style: { maxHeight: isOpen ? "5000px" : "0px", overflow: "hidden" }, children });
};
